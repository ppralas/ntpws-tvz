<?php 
	...
	if (isset($_POST['_action_']) && $_POST['_action_'] == 'add_book') {
		$_SESSION['message'] = '';
		$query  = "INSERT INTO books (title, description, archive)";
        ...
        if($_FILES['picture']['error'] == UPLOAD_ERR_OK && $_FILES['picture']['name'] != "") {
			...
			if ($ext == '.jpg' || $ext == '.png' || $ext == '.gif') {
				$_query  = "UPDATE books SET picture='" . $_picture . "'";
		...
		$_SESSION['flash_message'] .= '<p>You have successfully added a book!</p>';
		header("Location: index.php?menu=7&action=2");
	}
	
	if (isset($_POST['_action_']) && $_POST['_action_'] == 'edit_books') {
		$query  = "UPDATE books SET title='" . htmlspecialchars($_POST['title'], ENT_QUOTES) . "', description='" . htmlspecialchars($_POST['description'], ENT_QUOTES) . "', archive='" . $_POST['archive'] . "'";
		...
		$_SESSION['message'] = '<p>You have successfully changed a book!</p>';
	...
	if (isset($_GET['delete']) && $_GET['delete'] != '') {
        ...
        $query  = "SELECT picture FROM books";
		...
		$_SESSION['message'] = '<p>You have successfully deleted from table books!</p>';
	...
	if (isset($_GET['id']) && $_GET['id'] != '') {
		...
		print '
		<h2>Add Book</h2>
		<form action="" id="book_form" name="book_form" method="POST" enctype="multipart/form-data">
		
			<input type="hidden" id="_action_" name="_action_" value="add_book">
		...
		print '
		<h2>Edit books</h2>
		<form action="" id="book_form_edit" name="book_form_edit" method="POST" enctype="multipart/form-data">
			<input type="hidden" id="_action_" name="_action_" value="edit_books">
		...
		<h2>Books</h2>
			...
			<a href="index.php?menu=' . $menu . '&amp;action=' . $action . '&amp;add=true" class="AddLink">Add a new book</a>
		</div>';
	}
